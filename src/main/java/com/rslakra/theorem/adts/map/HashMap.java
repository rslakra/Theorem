 ...
 * @created 4/29/22 12:38 PM
 */
public class HashMap<K, V> implements Map<K, V> {

    private static final int DEFAULT_SIZE = 16;
    private LinkedList[] hashEntries;
    ...
    public HashMap(final int capacity) {
        ...
        hashEntries = new LinkedList[this.capacity];
        size = 0;
    }
    ...
    public boolean containsKey(Object key) {
        ...
        LinkedList<HashEntry> linkedEntry = hashEntries[hashIndex];
        HashEntry hashEntry = null;
        if (linkedEntry != null) {
            hashEntry = linkedEntry.peek();
    ...
    public V get(final Object key) {
        ...
        return (hashEntry != null && hashEntry.getKey().equals(key) ? (V) hashEntry.getValue() : null);
    }
    ...
    public V put(K key, V value) {
        int hashIndex = getHashIndex(key);
        if (hashEntries[hashIndex] == null) {
            hashEntries[hashIndex] = new LinkedList<>();
        }

        HashEntry oldValue = hashEntries[hashIndex].isEmpty() ? null : (HashEntry) hashEntries[hashIndex].getLast();
        hashEntries[hashIndex].add(new HashEntry<>(key, value));
        size++;
    ...
    @Override
    public void clear() {
        hashEntries = new LinkedList[capacity];
        size = 0;
    }
...
//        return Arrays.stream(hashEntries).map(hashEntry -> hashEntry).collect(Collectors.toSet());
    ...
    private class HashEntry<K, V> implements Map.Entry<K, V> {
        ...
        /**
         * Replaces the value corresponding to this entry with the specified value (optional operation).  (Writes
         * through to the map.)  The behavior of this call is undefined if the mapping has already been removed from the
         * map (by the iterator's {@code remove} operation).
         *
