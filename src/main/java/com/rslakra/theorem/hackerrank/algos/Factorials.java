/******************************************************************************
 * Copyright (C) Devamatre Technologies 2017
 *
 * This code is licensed to Devamatre under one or more contributor license 
 * agreements. The reproduction, transmission or use of this code or the 
 * snippet is not permitted without prior express written consent of Devamatre. 
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the license is distributed on an "AS IS" BASIS, WITHOUT 
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied and the 
 * offenders will be liable for any damages. All rights, including  but not
 * limited to rights created by patent grant or registration of a utility model 
 * or design, are reserved. Technical specifications and features are binding 
 * only insofar as they are specifically and expressly agreed upon in a written 
 * contract.
 *
 * You may obtain a copy of the License for more details at:
 *      http://www.devamatre.com/licenses/license.txt.
 *
 * Devamatre reserves the right to modify the technical specifications and or 
 * features without any prior notice.
 *****************************************************************************/
package com.rslakra.theorem.hackerrank.algos;

import java.math.BigInteger;
import java.util.Scanner;

/**
 * @author Rohtash Lakra
 * @version 1.0.0
 * @created 2017-12-06 08:16:56 AM
 * @since 1.0.0
 */
public class Factorials {

    /**
     * You are given an integer N. Print the factorial of this number. N! = N * (N - 1) * (N - 2) * ... * 3 * 2 * 1
     * <p>
     * Input: Input consists of a single integer N, where 1 <= N <= 100.
     * <p>
     * Output: Print the factorial of N.
     * <p>
     * Example For an input of 25, you would print 15511210043330985984000000.
     * <p>
     * Note: Factorials of can't be stored even in a long long variable. Big integers must be used for such
     * calculations. Languages like Java, Python, Ruby etc. can handle big integers, but we need to write additional
     * code in C/C++ to handle huge values.
     *
     * @param number
     * @return
     */
    public static BigInteger extraLongFactorial(int number) {
        if (number == 1) {
            return BigInteger.valueOf(1);
        } else {
            return BigInteger.valueOf(number).multiply(extraLongFactorial(number - 1));
        }
    }

    /**
     * @param args
     */
    public static void main(String[] args) {
        Scanner in = new Scanner(System.in);
        int n = in.nextInt();
        System.out.println(extraLongFactorial(n).toString());
        in.close();
    }

}
