 ...
 *******************************************************************************/
package com.rslakra.theorem.algos.list;
...
public class ListIsPreorder {
     ...
     * verified to be valid (valid only when parent of node[i] is in stack. In preorder a parent must show up earlier
     * than its child) Whenever stack top is not the parent of node[i], pop until parent of node[i] is at stack top.
     * Push node[i]. If all nodes popped but parent of node[i] still not found, then node[i] is not in preorder
     * sequence.
     *
     * @return
     */
